...
public class ReductionExamples {
	...
	public static void main(String... args) {
		...
		Integer totalAgeReduce = roster.stream().map(Person::getAge)
				.reduce(0, (a, b) -> a + b);

		System.out.println("Sum of ages with reduce(identity, accumulator): "
				+ totalAgeReduce);
		...
		Map<Person.Sex, List<Person>> byGender = roster.stream().collect(
		...
		List<Map.Entry<Person.Sex, Integer>> totalAgeByGenderList = new ArrayList<>(
				totalAgeByGender.entrySet());

		totalAgeByGenderList.stream().forEach(
				e -> System.out.println("Gender: " + e.getKey()
