...
public class BitComputTest extends TestCase {
	...
	/**
	 * A = 0011 1100<br>
	 *  << 按位左移运算符。左操作数按位左移右操作数指定的位数。<code>	A << 2得到240，即 1111 0000</code> <br>
	 *  <pre> 向左移n位相当于是<b>乘</b>以2<sup>n</sup>次方</pre>
	 *  
	 *  >> 	按位右移运算符。左操作数按位右移右操作数指定的位数。	<code>A >> 2得到15即 1111</code> <br>
...
//		}
		...
		flags = flags | flagbitN;
		System.out.println(flags); //3
		
		//Check if a bit is set
		System.out.println((flags & flagbitN) == flagbitN);//true
		
		//Unset a flag (zero-out a bit or set a bit to zero)
		flags = flags & ~flagbitN;
		System.out.println(flags);//2
		
		System.out.println((flags & flagbitN) == flagbitN);//false
